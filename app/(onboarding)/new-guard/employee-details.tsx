...
import { View, StyleSheet, ScrollView, Platform, Animated, useWindowDimensions } from 'react-native';
...
export default function EmployeeDetailsScreen() {
  ...
  const isStepValid = useCallback((step: number, data: EmployeeFormData) => {
    switch (step) {
      case 1:
        return (
          data.dateOfJoining.length === 10 && data.unitSite.length > 0 &&
          data.firstName.length > 0 && data.surname.length > 0 && data.fatherName.length > 0 &&
          data.gender.length > 0 && data.dateOfBirth.length === 10 &&
          data.mobileNumber.length === 10 && data.bloodGroup.length > 0
        );
      case 2:
        return (
          data.aadhaarNumber.length === 12 && data.panNumber.length === 10 &&
          data.uanNumber.length === 12 && data.esicNumber.length === 17
        );
      case 3:
        return (
          data.permanentAddress.length > 0 && data.currentAddress.length > 0 &&
          data.city.length > 0 && data.state.length > 0 && data.pinCode.length === 6 &&
          data.bankName.length > 0 && data.accountNumber.length > 0 &&
          data.ifscCode.length === 11 && data.branch.length > 0 && data.micrCode.length === 9
        );
      case 4:
        return (
          data.em1Name.length > 0 && data.em1Relation.length > 0 && data.em1Mobile.length === 10 &&
          data.em2Name.length > 0 && data.em2Relation.length > 0 && data.em2Mobile.length === 10
        );
      default:
        return false;
    }
  ...
  const isCurrentStepValid = useMemo(() => isStepValid(currentStep, formData), [currentStep, formData, isStepValid]);
  ...
  const handleFinalSubmit = () => {
    updateData({
      ...
      identity: {
        ...
        esic: formData.esicNumber, pf: formData.pfNumber,
      },
      address: {
      ...
      bank: {
        bankName: formData.bankName, accountNumber: formData.accountNumber, ifsc: formData.ifscCode,
        branch: formData.branch, micr: formData.micrCode,
      },
      emergencyContacts: [
        ...
        <View style={styles.sliderWrapper}>
          <Animated.View style={[
            ...
            <View style={{ width: stepWidth }}>
              <EmploymentPersonalStep formData={formData} updateField={updateField} currentYear={currentYear} />
            </View>
            <View style={{ width: stepWidth }}>
            ...
            <View style={{ width: stepWidth }}>
              <EmergencyContactStep formData={formData} updateField={updateField} />
            </View>
          </Animated.View>
        </View>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  scrollView: { flex: 1 },
  scrollContent: { flexGrow: 1, paddingTop: spacing.sm, paddingBottom: spacing.lg },
